...
> 📦 **Where the code lives** (updated 2026-08-30): the plugin contract SDK is in
> [velashell-plugin-sdk](https://github.com/VelaShellLabs/velashell-plugin-sdk); the
> `vela-plugin` CLI and `VelaShell.PluginSdk.Build` are in
> [velashell-plugin-cli](https://github.com/VelaShellLabs/velashell-plugin-cli); the
> `dotnet new` templates are in
> [velashell-plugin-templates](https://github.com/VelaShellLabs/velashell-plugin-templates);
> the Redis / S3 / Telnet / Serial plugins are in
> [velashell-plugins](https://github.com/VelaShellLabs/velashell-plugins).
> Paths such as `plugin-sdk/…`, `tools/…` and `templates/…` on this page (and in blueprints
> 01–15) refer to the first three; `tests/VelaShell.Plugin.{Redis,S3,Telnet}.Tests` to the fourth.
>
...
> host loads). The reasoning is in [`plugins/README.md`](https://github.com/joesdu/VelaShell/blob/main/plugins/README.md).
...
> Updated: 2026-08-12. This page is the **single source of truth** for implementation progress: each blueprint area is listed as complete / partially complete / not started, with acceptance evidence (tests) and recommended next steps. To write a plugin, read [dev-guide.md](../templates/dev-guide.md).
...
**The first first-party business plugin has shipped: the AI assistant plugin** (`plugins/VelaShell.Plugin.Ai`, ID `velashell.ai`): multi-provider streaming conversations (three wire protocols—OpenAI Responses / OpenAI Chat Completions compatible / Anthropic Messages—covering OpenAI/Grok/Ollama/relay services, user-supplied Base URL + API Key, with the key encrypted through Secrets) + Agent mode (Microsoft.Extensions.AI `FunctionInvokingChatClient` tool loop, bridging sessions/terminal/remoteExec/remoteFs, with dangerous operations approved individually in the tool panel). Lazy activation through `onCommand`, with five-language copy. Acceptance: 14 `VelaShell.Plugin.Ai.Tests` cases (toolbox approval gate/capability-bridge semantics/settings and secret access). **Collaboration (2026-09-02)**: the same plugin grew two routes that point in opposite directions — outbound is the **chat bridge** (Feishu / DingTalk / Telegram / WeCom, each on a different inbound transport: long connection / Stream mode / long polling / public callback), where teammates @ the bot in a group, the agent works on an already-connected session and posts the answer back; inbound is the **outbound MCP server** (loopback-only + mandatory token + read-only default mode) that lets Claude Code / Codex call VelaShell tools, reusing `AgentToolbox` directly. The defaults are deliberately conservative: an empty allowlist serves nobody, the bridge defaults to the read-only mode, `/mode` may only lower the privilege, approvals are text replies (a timeout counts as refused), and approvers can be configured separately from who may talk. Authorising a group needs no copying of ids — generate a six-digit pairing code and send `/pair` in the chat, or press Allow on the "chats that tried to talk to the bot" list. **Working on machines that are not connected (2026-09-03)**: `AgentToolbox` gained `list_saved_sessions` / `open_session` / `close_session` (SDK 2.0.2 plus the host implementation), so when whoever was on duty closed the tab last night the bot can connect one of the **saved configurations** itself instead of answering "connect one first". `open_session` is the only tool that goes past two humans: this turn's approval plus the host's own confirmation dialog (which shows the agent's stated reason verbatim and offers "always allow" — that is what makes the unattended route work); it is not registered in plan mode, and on the MCP route it only works under "bypass approval". `close_session` needs no approval but can only close what the plugin itself opened. See [Collaboration](collaboration.md). **The container-management plugin is complete (2026-09-03, `0.3.1`)**: [VelaShell.Plugin.DockerPanel](https://github.com/VelaShellLabs/velashell-plugins/tree/main/plugins/VelaShell.Plugin.DockerPanel) — it opens a direct channel to the remote `/var/run/docker.sock` over the SSH session and speaks the Docker Engine HTTP API (nothing changes on the server: the daemon need not be exposed on 2375/2376 and no second set of credentials is required). Nor is it a local port forward, so no other process on the machine can reach it. Seven pages — overview / containers / images / volumes / networks / Compose / system — plus live stats, a merged multi-container log stream, in-container file browsing and editing, and a built-in TTY console. Not preinstalled; installed on demand from the marketplace.
...
| Capability domains: sessions/remoteFs/remoteExec | 07 | Reuse host connections, throttle progress updates, and enforce semantics such as returning null for a missing path from Stat. **Opening a saved session (2026-09-03, SDK 2.0.2)**: `ListSavedAsync` / `OpenAsync` / `CloseAsync` landed on the host side — only saved configurations can be opened, the stated reason goes into the confirmation dialog verbatim, not one byte of credentials passes through the plugin, and a successful connection runs the host's normal connect flow so a real tab appears (credential prompt, jump-host chain, fingerprint confirmation and connection history all included); "the user refused" and "it did not connect" are two outcomes with two exception types; a plugin can only close what it opened | Contract-level tests + e2e, SessionsCapabilityTests, SessionRoutingTests |
...
| First-party business plugins | 15 | **No longer belongs in this table**: the AI assistant (ships with the host), Redis / S3 / Telnet / serial ([velashell-plugins](https://github.com/VelaShellLabs/velashell-plugins)) and the Docker panel ([VelaShell.Plugin.DockerPanel](https://github.com/VelaShellLabs/velashell-plugins/tree/main/plugins/VelaShell.Plugin.DockerPanel), `0.3.1`) have all shipped |
