...
> 📦 **Where the code lives** (updated 2026-08-30): this directory is the **host-side design
> blueprint** for the plugin system; the code sits in several repositories — the plugin contract
> SDK is in [velashell-plugin-sdk](https://github.com/VelaShellLabs/velashell-plugin-sdk);
> the `vela-plugin` CLI and `VelaShell.PluginSdk.Build` are in
> [velashell-plugin-cli](https://github.com/VelaShellLabs/velashell-plugin-cli);
> the `dotnet new` templates are in
> [velashell-plugin-templates](https://github.com/VelaShellLabs/velashell-plugin-templates);
> the first-party plugins (Redis / S3 / Telnet / Serial) are in
> [velashell-plugins](https://github.com/VelaShellLabs/velashell-plugins);
> the host itself is [joesdu/VelaShell](https://github.com/joesdu/VelaShell).
> On this page (and in blueprints 01–15), `plugin-sdk/…`, `tools/…` and `templates/…` refer to the
> first three; `plugins/VelaShell.Plugin.…` and `tests/VelaShell.Plugin.*.Tests` refer to the fourth.
> **Exception: the AI plugin stays in the host repository** (`plugins/VelaShell.Plugin.Ai`, tests in
> `tests/VelaShell.Plugin.Ai.Tests`) and is built and released with the host — see
> [plugins/README.md](https://github.com/joesdu/VelaShell/blob/main/plugins/README.md).
...
>
> **To write a plugin, read [dev-guide.md](../templates/dev-guide.md) directly** — it describes the APIs that are actually available today; the interface shapes in the blueprint documents (one process per plugin, Broker, etc.) are not implemented in v1.
...
| --- | --- | --- |
| [dev-guide.md](../templates/dev-guide.md) | **Development guide (implemented)**: quick start, manifest, lifecycle, capability APIs, isolation modes, testing, deployment, performance discipline | Plugin developers (required reading) |
| [cli.md](../cli/cli.md) | **`vela-plugin` manual**: development inner loop (`dev init`), health check (`doctor`), validate/pack/sign, host launch arguments | Plugin developers |
...
- **Writing a plugin**: [dev-guide.md](../templates/dev-guide.md) (the sole authority for the current implementation), with [cli.md](../cli/cli.md) and [sdk-reference.md](../sdk/sdk-reference.md) at hand; read [publishing.md](../templates/publishing.md) when you are ready to ship.
