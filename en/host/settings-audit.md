...
| R-06 | P1 | Load keys into Agent automatically | ~~Disabled, unimplemented~~ Implemented: after a successful private-key-file authentication, the key is added to the local ssh-agent (like OpenSSH `AddKeysToAgent`) | Show again | `KeyOptions.AddKeysToAgent` now has a runtime consumer | ✅ Returned to the UI (2026-09-23, "SSH Agent" section of the Key Management page). The field was renamed from `AutoLoadToAgent` to `AddKeysToAgent` and is **off by default**: the old field defaulted to `true` but never had a consumer, so the `true` in existing configurations is not user consent. Pageant is still not supported |
