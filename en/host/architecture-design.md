...
| Port-forwarding tunnels (service layer) | ✅ | `Infrastructure/Tunnels/TunnelService.cs`; the forwarding data path is host-owned in `Infrastructure/Ssh/MeteredPortForwardHandle.cs` (with `Socks5Negotiation`), since the library exposes no counters |
