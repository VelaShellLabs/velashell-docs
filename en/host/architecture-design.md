...
| Port-forwarding tunnels (service layer) | ✅ | `Infrastructure/Tunnels/TunnelService.cs`; both the forwarding data path and the metering live in the SSH library (`LocalPortForwarder` / `RemotePortForwarder`), and the host adapts them thinly through `Infrastructure/Ssh/LibraryPortForwardHandle.cs` |
...
- **The composition string must be drawn by the client (`SupportsPreedit`)**: Avalonia's Win32 backend **never** lets the IME draw its own composition window (`Imm32InputMethod.ShowCompositionWindow` is always `false`; it only calls `ImmSetCandidateWindow` to position the candidate window), so the only outlet for the composition string is `TextInputMethodClient.SetPreeditText` — and that call is gated behind `Client.SupportsPreedit`. The client **must** therefore report `SupportsPreedit = true` and render the composition string (pinyin/kana/jamo) at the cursor itself; otherwise the platform silently drops it: the candidate window floats next to the cursor while nothing appears on screen, so a user who mistypes cannot tell how many characters to delete. The composition string **never enters the screen buffer and is never sent to the PTY**; like ghost text it is a purely visual overlay on `CursorOverlay`. Its column widths come from the same `CharWidth` used by the main text (the same metric as the echoed result, so nothing shifts horizontally on commit), it carries an underline to mark "not yet committed", and the cursor moves into the composition at the caret offset the IME reports (IMM32's `GCS_CURSORPOS`) while keeping whatever style the user configured. The cursor rectangle reported to the platform spans the whole composition while composing — Avalonia uses it as the candidate window's `CFS_EXCLUDE` area, so reporting a single cell would let the candidate window cover the pinyin.
...
- **Password encryption (implemented)**: `ISecretProtector` (`Core` interface) + `AesSecretProtector` (AES-256-GCM, key file `~/.velashell/secret.key`). Session passwords/private-key passphrases are encrypted before being written to SonnetDB. Legacy plaintext is supported on read and automatically encrypted on the next save. Credentials are not persisted when “Remember password” is unchecked.
