...
10. **空组折叠**:拆分出的组关掉最后一个标签后自动移除并提升兄弟节点;**根**永不消失。
    (2026-09-10 修正:原文写的是“主组永不消失”。把这条豁免钉在某个具体实例上,会让被拖到
    侧边、又被清空的主组变成一块赶不走的空白 —— 分屏后关光左半屏,右半屏填不满整片区域。
    现在“主组”只是一个**可以易主**的兜底身份,清空时交给幸存的邻居。见宿主仓 `plan.md` §64。)
...
- **结构操作**(全部在模型层,可单测):
  - `AddDocument(doc)`:放进最合适的窗格并激活。落点三级:**空窗格 > 当前正在用的窗格 > 主组**
    (2026-09-10 起;原为“总进主组”,分屏后会把新标签开在人没在看的那半边)。
  ...
  - 空组折叠:任何组因“文档移出/关闭”清空 → 从父分栏移除(主组先把兜底身份交给幸存的邻居,
    见上文 §1.2-10);分栏仅剩 1 子 → 子节点提升(比例继承);提升出的若是拆分留下的空组
    (兄弟已全部关闭)一并回收;根分栏收敛回单组。**只有根不折叠** —— 布局树总得有个底。
  - 拆分留下的空组保留在原位作为放置目标,面板上给“把其他窗格的标签页拖到这里”+ 一枚
    「关闭窗格」(`ClosePane`)。一个会话都没开时(根窗格)换成另一套文案且**不给按钮** ——
    那时既没有标签可拖,也没有窗格可关。
  - `ToggleMaximizeGroup(group)` / `MaximizedGroup`:窗格最大化(tmux 的 `resize-pane -Z`),
    **只影响渲染、不动布局树**;焦点切走 / 该窗格离开树 / 布局收敛回一格时自动解除。
  - `EqualizePanes()`:所有分栏比例复位为均分(`Proportion = NaN`)。
