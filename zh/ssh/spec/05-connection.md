...
| --- | --- |
| 初始 | 256 KiB |
| 慢启动 | 每个 RTT 内窗口被完全用尽 → 窗口 ×2，直到 `max` 或不再用尽 |
| 稳态 | 连续 3 个 RTT 未用尽 → 窗口 ×0.75（下限 `min`） |
| RTT 估计 | 从 keepalive 全局请求与 SFTP 应答的往返时间取指数移动平均 |
...
2. **两条流必须能被独立消费而不互相饿死。** 一次性命令要同时收 stdout 与 stderr：
   如果只有一个读接口，调用方轮流读两边，一边读空时另一边可能正在被对端写满
   —— 那就是经典的双管道死锁。两条独立的 `PipeReader` 加各自的缓冲，
   让「一边不读」只影响那一边的窗口。
...
5. 超时的探测**仍然留在全局请求账本里**（§6.1）：应答只是迟到，
   它到的时候必须落在这一项上，后面真正的全局请求才对得上号。
6. 判死之后会话**真的停下来**：放出 `Disconnected`、停收发泵、关掉所有通道
   （通道的读者读到结尾，而不是永远挂着）。
