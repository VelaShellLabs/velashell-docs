...
**值得做，但理由不是「想自研」，而是 VelaShell 已经为绕开底层库写了 1,200 余行补丁，
...
- 真正的风险**不在法律，在三处别的地方**：
  ...
  3. **升级绝缘** —— fork 之后上游每次改动你都要手工 rebase，几个月就漂到无法合并，
     最后变成「既没有自研的自由，也没有上游的红利」。**这是 fork 最贵的隐性成本。**
...
### 2.4 CI 相似度门禁（这条最有说服力）

在 CI 里加一个 job：把 Tmds.Ssh 的源码拉下来（只在 CI 的临时目录，**不进仓库**），
对我们的 `src/` 做 **token 级 n-gram 指纹比对**（winnowing / MOSS 式算法，
或直接用 `jscpd` / `simian` 的 token 模式）：

```
门槛：任意单文件对的最大公共 token 序列 ≤ 40 token
      全仓 token 级相似度 ≤ 1%（协议常量表与算法名清单加入白名单）
超阈值 → 构建红灯，并打印命中片段
```

这条门禁的价值不只是防呆：**它是一份可以直接交给对方的证据**。
有人质疑时，回答不是「我们没抄」，而是「每次提交都自动比对，阈值在这，历史记录在这」。

**当前实测（2026-09-21，语料 = Tmds.Ssh 197 个 .cs + SSH.NET 330 个 .cs，
共 502 文件 / 262,929 token）**：

```
覆盖率   0.16%   (阈值 1.00%)
超限文件 0       (单文件最长公共串阈值 40 token)
自检     99.81% 覆盖、174 文件超限、退出码 1   ← 拿语料比对它自己
底噪     2.41%                                ← SSH.NET 对 Tmds.Ssh
```

**「底噪」那一行是整节里最有分量的数字。** 它回答的是一个本来只能靠嘴说的问题：
*两个确实互不相干的 SSH 实现，本来会有多像？* SSH.NET 与 Tmds.Ssh 没有共同血缘，
光是「都在直译同一批 RFC」就重合 **2.41%**。我们对同一份语料是 **0.16%** ——
**比两个无关实现之间的底噪还低一个数量级。**

标定完之后阈值从 3% 收到了 1%。不是「更严更好」：3% 压在底噪上方，
它说的只是「我们不比两个陌生实现之间更像」，几乎不传递信息；
1% 在底噪**之下**，是一条盯住**我们自己**的早期警报线 ——
现在 0.16%，余量 6 倍有余，真涨到 1% 就说明有东西变了。

**自检那一行才是这个数字有意义的前提。** 一个永远报 0% 的检测器和没有检测器
是一回事 —— 我自己就踩过：本机 `.corpus/` 是空的（它不进仓库），
跑出来一个漂亮的 `0.00%`，而那只是「拿 0 个文件比对」的结果。
现在 CI 与本机都先跑自检：**拿语料当「我们的代码」比它自己，这一步必须红**，
红不了就说明门禁已经失效，后面那次真正的比对也不用看了。

三类误报都是**改进检测器**解决的，不是抬阈值：

| 命中 | 为什么不是抄袭 | 怎么处理 |
| --- | --- | --- |
| SFTP 四张码表（110 token） | `draft-ietf-secsh-filexfer-02` 规定的名字与数值 | 拆成 `SftpProtocolCodes.cs` 进 allowlist，**其余常量留在原文件继续受监视** |
| `Stream` 子类的转发样板（82 token） | 签名由基类定死，方法体只有一种写法 | 进 `boilerplate.cs.txt` —— 它剔除的是**那几个 k-gram**，不是整个文件 |
| 算法名字符串 | RFC 4253 / OpenSSH PROTOCOL 的字面量 | 同上 |

区别很重要：**allowlist 放行整个文件**（所以只给纯码表用，且要拆到最小），
**boilerplate 只剔除特定片段**（同一文件里真出问题照样报）。
...
| CI（构建测试 · 公开面门禁 · 相似度门禁 · 打包冒烟 · interop） | ✅ 已写，未在真实 runner 上跑过。interop 当时是占位，现已接真容器（§11.2.9） |
| **相似度门禁** + 自检 | ✅ 自检实测：对照抄报 99.81% 覆盖、175/197 文件超限、退出码 1 |
...
1. **收敛成两个项目**（库 + 测试），内部用文件夹分层 —— 原计划是 Core + 主包两个包。
   理由见 §4。
2. **相似度门禁已有实测基线**：对 Tmds.Ssh 的重合度 **0.00%**，阈值从 40 token 收紧到 20 token 仍是 0.00%。
   过程中抓到一条 38 token 的误报（`Stream` 非 Seek 样板），因此新增了片段级的样板豁免机制。
   ⏳ 仍缺**跨库底噪标定**（`--max-ratio 3%` 的依据）（开发机连不上 github.com，
   拉不到第二个参照实现）。在标定完成前，真正起作用的是 `--max-run`（单文件最长公共串），
   那个指标对底噪不敏感。详见 `scripts/ssh/similarity-gate/README.md`。
...
这里有一个**必须点破的陷阱**：常规门禁的语料是 Tmds.Ssh 的 **main 分支**，
而 #513 还没合并 —— **它的文件根本不在语料里**。
也就是说「跑了门禁、通过了」对这一次改动**什么都没证明**。
一个量不到目标的检测器和没有检测器是一回事（§2.4 已经栽过一次）。

所以另外拿 PR 自己的 25 个 `.cs` 建了一份临时语料，直接比：

```
                     覆盖率    最长公共串（正式阈值 40）
第一版实现            0.44%    101 token  ← 超限，门禁拒绝
Stream 适配器重写后    0.27%     36 token  ← 通过
```

第一版那 101 token 是 `Stream` 适配器的尾巴
（`Read`/`Flush`/`Seek`/`SetLength` 那一串 `NotSupportedException`）。
内容确实全是 BCL 形状逼出来的 —— 证据是
`LinkCharacteristics.cs`（39 token）与 `InMemoryTransport.cs`（31 token）
也同样命中，**而那两个文件写在我打开这个 PR 之前**。

但 101 个连续相同的 token 正是门禁要抓的东西，而我**确实是刚看完那份文件才写的**，
没法诚实地声称成员顺序没受影响。所以修法不是加白名单（那等于把信号藏起来），
而是**照我们自己的风格重写**：把真正做事的成员放最前、
拒绝的成员统一走一个带解释的 `NotAStream(member)` 工厂 ——
这同时也更符合架构原则 3（失败要带数据），裸抛一个 `NotSupportedException`
本来就不如写清楚「为什么这里不该被调用」。

重写之后 `SshCompressor.cs` 的最长公共串是 36 token，
**比写在读 PR 之前的 `LinkCharacteristics.cs`（39）还短**。
主语料上的覆盖率从 0.17% 降到 **0.16%**。

> 记一句：**净室纪律挡的是「照抄别人的结构」，不是「不许知道事实」。**
> `Z_SYNC_FLUSH` 不重置字典是 RFC 1951 的事实，从哪儿知道的都一样。
> 区别在于知道之后是自己写，还是把别人的文件拷过来。
>
> 但要让这句话站得住，**检测器必须真的能看到你参考的那份东西** ——
> 否则「门禁通过」只是一句自我安慰。
...
拿那个 PR 的 20 个 `.cs` 建临时语料直接比：

```
覆盖率   0.00%    最长公共串  < 15 token（收到 15 仍然是 0 个超限文件）
```

比 §11.2.10 那次（第一版 101 token，重写后 36）干净得多 ——
那次是先照着写再回头拆，这次是先写规格（`spec/07` §7.5）再实现。
...
也正是当初决定自研的两个理由之一）：`TerminalSize` 四个字段贯通
...
| 「还是像」 | §2.4 的相似度门禁从 M0 第一次提交就开 —— 不是最后来补，那时候已经晚了 |
