...
> 适用版本:vela-plugin **2.0.2**(`vela-plugin --version` 看本机装的是哪版)。
> 相关文档(在别的仓库):[开发指南](../templates/dev-guide.md) · [打包与发布](../templates/publishing.md) · [SDK 参考](../sdk/sdk-reference.md)
...
宿主第一次看见这样一个目录时,会把它当时的内容记成基线并正常加载 —— 所以命令行装完重启即可用。
那份基线只是"我第一次见到它时长这样":以后 `vela-plugin update` 换了内容,宿主重新记一遍,
不会把插件标红(这正是"没有事后防篡改"的意思)。

> **别把两条路混着用同一个插件。** 经管理页装的插件,收据是宿主在解包之后亲手落下的,
> 它确实等于"这个目录出自那个包";再用命令行覆盖装上去,宿主只会看到内容变了,
> 于是判成"文件被改过"并标成 Invalid。要换用命令行,先在管理页把它卸载掉。
