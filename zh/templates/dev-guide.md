...
工具链自 2026-08-27 起按发布节奏拆成三个仓库,各有各的版本号:
...
重启宿主即加载。搭配 `search` / `list` / `update` / `uninstall`,详见 [CLI 手册](../cli/cli.md#2-从插件商店安装)。
...
> 命令行造不出那份收据(宿主改为把这类目录第一次见到时的内容记成基线,重启即加载),
> 但装之前的检查一条不少。同一个插件别把两条路混着用 —— 覆盖一个经管理页装的插件会被判成
> "文件被改过"。两边的取舍见 CLI 手册。
...
[CLI 手册](../cli/cli.md);这里说清它到底做了什么、为什么这么做。
...
```xml
<!-- csproj:版本 = 宿主版本(当前 12.1.3);Avalonia dll 绝不复制进插件目录,
...
**窗口模式同样吃这个图标**:它画在自绘标题栏最左边,进程内与隔离进程都是。
几个插件的设置窗口并排开着时,标题栏顶着各自的标才分得清哪扇是谁的 ——
在这之前那里进程内一律是通用插头、隔离进程干脆什么都没有。

路径畸形不会把界面顶掉:宿主解析失败时当作没给,退回通用插头。

⚠️ 别和上面的 `TitleActions` 搞混:那个画的是标题栏**右侧的动作按钮**,恒为 24×24 描边,
没有填充与视框可言;这个画的是标签页与标题栏**最左边**那一个。
