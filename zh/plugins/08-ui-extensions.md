...
> **实现注记(2026-08)**:按用户决策,VelaUI 远程界面树**不做**(曾短暂实现后移除)——
> 插件 UI 直接使用**完整的 Avalonia**(编译期 AXAML/代码、自带样式与国际化、可引入
> 第三方组件包),约束仅为 Avalonia 版本与宿主一致(装载器按 `Avalonia*` 前缀共享)。
> 进程内插件的面板可停靠进主窗口标签区(真·dock 标签,可拖拽分栏);隔离进程插件
> 由 PluginHost 内建 Avalonia 呈现为**独立卡片窗口**(PluginHostShellWindow,与主程序
> 资源监视窗口同规格)。跨进程 dock 嵌入(HWND 收养 SetParent)曾实现,但与 dock 的
> 单宿主 reparenting 有根本张力(切标签反复摘挂跨进程窗口 → 卡顿/窗口飘出),**已弃用**
> 并移除宿主 Win32 实现;仅保留 `ui/embed` RPC 协议供未来复用。跨平台的停靠嵌入稳态
> 方案仍是本篇 §4 的共享内存图像表面(远期)。现状 API 见 dev-guide §5.9。
...
见 07 §9(能力域),UI 侧只有状态栏/面板由插件用 L1/L2 自己创建。
