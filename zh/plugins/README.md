...
> 📦 **代码仓库对照**(2026-08-30 更新):本目录是插件系统的**宿主侧设计蓝图**,
> 代码分散在几个仓库里 —— 插件契约 SDK 在
...
> 第一方插件(Redis / S3 / Telnet / Serial)在
> [velashell-plugins](https://github.com/VelaShellLabs/velashell-plugins);
> 宿主实现在 [joesdu/VelaShell](https://github.com/joesdu/VelaShell)。
> 本页(以及 01–15 蓝图)里出现的 `plugin-sdk/…`、`tools/…`、`templates/…` 指前三者,
> `plugins/VelaShell.Plugin.…`、`tests/VelaShell.Plugin.*.Tests` 指第四个。
> **例外:AI 插件仍在宿主仓库**(`plugins/VelaShell.Plugin.Ai`,测试在 `tests/VelaShell.Plugin.Ai.Tests`),
> 随主程序一起构建发布,理由见 [plugins/README.md](https://github.com/joesdu/VelaShell/blob/main/plugins/README.md)。
...
>
> **写插件请直接读 [dev-guide.md](../templates/dev-guide.md)** ——
...
| 文档 | 内容 | 读者 |
| --- | --- | --- |
| [dev-guide.md](../templates/dev-guide.md) | **开发指南(已实现)**:快速上手、清单、生命周期、能力 API、隔离模式、测试、部署、性能纪律 | 插件开发者(必读) |
| [cli.md](../cli/cli.md) | **`vela-plugin` 手册**:开发内环(`dev init`)、体检(`doctor`)、校验/打包/签名、宿主启动参数 | 插件开发者 |
...
- **写插件**:[dev-guide.md](../templates/dev-guide.md)(现状唯一权威)→ 手边备着
  [cli.md](../cli/cli.md) 与 [sdk-reference.md](../sdk/sdk-reference.md);要发布时读 [publishing.md](../templates/publishing.md)。
